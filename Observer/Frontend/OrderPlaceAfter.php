		...
		if($couponCode == $referralCode){
			...
			if(count($quotecouponCollection)){
				...
				if(count($referralCollection)){
					...
					$usedCouponCollection->addFieldToFilter('order_id', $order->getId());
					if(count($usedCouponCollection)){
						$usedCouponModel = $usedCouponCollection->getFirstItem();
						$usedCouponModel->setCustomerId($customerId);
						try{
							$usedCouponModel->save();
						}catch(Exception $e){
					...
					$earnedCollection = $this->_earnedFactory->create()->getCollection();
					$earnedCollection->addFieldToFilter('order_id', $order->getId());
					if(count($earnedCollection)){
						$earnedModel = $earnedCollection->getFirstItem();
